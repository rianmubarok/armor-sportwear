...
@section('page-subtitle', 'Perbarui detail pesanan tim/borongan')

@section('content')
<div class="w-full">
    <div class="bg-white border border-[#D0D0CC]">
        <div class="px-6 py-4 border-b border-[#D0D0CC] bg-[#F8F8F6]">
            <h2 class="text-xl font-bold text-[#1A1A1A] font-['Teko'] uppercase leading-none">Form Edit Pesanan #ORD-{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}</h2>
        </div>

        <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="p-6 space-y-8" id="orderForm">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="customer_name" id="customer_name" class="admin-input" value="{{ old('customer_name', $order->customer_name) }}" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="customer_phone" id="customer_phone" class="admin-input" value="{{ old('customer_phone', $order->customer_phone) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <input type="number" name="total_price" id="total_price" class="admin-input" value="{{ old('total_price', $order->total_price) }}">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="admin-input" required>
                            <option value="pending" {{ old('status', $order->status) == 'pending' ? 'selected' : '' }}>Pending (Menunggu DP/Pembayaran)</option>
                            <option value="processing" {{ old('status', $order->status) == 'processing' ? 'selected' : '' }}>Diproses (Produksi)</option>
                            <option value="completed" {{ old('status', $order->status) == 'completed' ? 'selected' : '' }}>Selesai (Terkirim)</option>
                            <option value="cancelled" {{ old('status', $order->status) == 'cancelled' ? 'selected' : '' }}>Batal</option>
                        </select>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="2" class="admin-input">{{ old('notes', $order->notes) }}</textarea>
                    ...
                    <table class="w-full text-left border-collapse" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            ...
                            <tr class="item-row border-b border-[#E0E0E0]">
                                <td class="p-2">
                                    <select name="items[{{ $index }}][product_id]" class="w-full text-sm border border-[#D0D0CC] p-2 focus:outline-none focus:border-[#1A1A1A] bg-[#F8F8F6]">
                                ...
                                <td class="p-2"><input type="text" name="items[{{ $index }}][custom_design]" value="{{ $item->custom_design }}" class="w-full text-sm border border-[#D0D0CC] p-2 focus:outline-none focus:border-[#1A1A1A] bg-[#F8F8F6]" placeholder="Kosongkan jika pilih produk"></td>
                                <td class="p-2"><input type="text" name="items[{{ $index }}][player_name]" value="{{ $item->player_name }}" class="w-full text-sm border border-[#D0D0CC] p-2 focus:outline-none focus:border-[#1A1A1A] bg-[#F8F8F6]" placeholder="Contoh: BUDI"></td>
                                <td class="p-2"><input type="text" name="items[{{ $index }}][player_number]" value="{{ $item->player_number }}" class="w-full text-sm border border-[#D0D0CC] p-2 focus:outline-none focus:border-[#1A1A1A] bg-[#F8F8F6]" placeholder="10"></td>
                                <td class="p-2">
                                    <select name="items[{{ $index }}][size]" class="w-full text-sm border border-[#D0D0CC] p-2 focus:outline-none focus:border-[#1A1A1A] bg-[#F8F8F6]">
                                        <option value="">-</option>
                                        @foreach(['S', 'M', 'L', 'XL', 'XXL', '3XL', '4XL'] as $sz)
                                            <option value="{{ $sz }}" {{ $item->size == $sz ? 'selected' : '' }}>{{ $sz }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td class="p-2"><input type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" class="w-full text-sm border border-[#D0D0CC] p-2 focus:outline-none focus:border-[#1A1A1A] bg-[#F8F8F6]" min="1"></td>
            ...
            <div class="pt-4 flex flex-col sm:flex-row justify-between gap-3 border-t border-[#D0D0CC]">
                <button type="button" onclick="if(confirm('Hapus pesanan ini secara permanen?')) { document.getElementById('form-delete-order').submit(); }" class="admin-btn-danger">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    Hapus Keseluruhan Pesanan
                </button>
...
<form id="form-delete-order" action="{{ route('admin.orders.destroy', $order) }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
</form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initial check if any name/number exists to set checkbox state
        let hasNameOrNumber = false;
        document.querySelectorAll('input[name$="[player_name]"]').forEach(input => {
            if (input.value.trim() !== '') hasNameOrNumber = true;
        });
        document.querySelectorAll('input[name$="[player_number]"]').forEach(input => {
            if (input.value.trim() !== '') hasNameOrNumber = true;
        });
        
        if (!hasNameOrNumber && itemIndex > 0) {
            // Only uncheck if explicitly empty
            toggleNameNumber.checked = false;
            updateNameNumberStatus();
        }
