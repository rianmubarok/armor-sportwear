...
@section('page-subtitle', 'Perbarui data katalog produk Anda.')

@section('content')
<div class="bg-white rounded-xl shadow-sm border border-gray-200 max-w-3xl">
    <div class="p-6 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-800">Form Edit Produk</h2>
    </div>
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="price_start_from" id="price_start_from" value="{{ old('price_start_from', $product->price_start_from) }}" min="0" required
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" required
                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $product->description) }}</textarea>
            @error('description') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        {{-- Image --}}
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Gambar Produk Saat Ini</label>
            @if($product->image)
                <img src="{{ Storage::url($product->image) }}" alt="Preview" class="h-32 object-cover rounded-lg border border-gray-200 mb-4">
            @endif
        ...
        <div class="pt-4 flex justify-end gap-3 border-t border-gray-200">
            ...
            <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-sm">Perbarui Produk</button>
        </div>
    </form>
</div>
@endsection
